...
if($isPost) {
    ...
    $gdflixString = $_POST['gdflix_links'];

    $pattern = '/<a href="([^"]+)">(.+?)<\/a>/i';

    preg_match_all($pattern, $gdflixString, $matches);

    // print_r($matches);
    
    $uid_gdflix = [];
    
    foreach($matches[2] as $index => $gdflix) {
        
        $name = preg_replace('/ \[\d+(\.\d+)? [A-Z]{2,3}\]$/', '', $gdflix);

        
        preg_match('/https:\/\/[a-z0-9.]+\/file\/([a-zA-Z0-9]+)/', $matches[1][$index], $slug_matches);
        $gdflix_slug = $slug_matches[1] ?? null;
        ...
        $uid_gdflix = ['gdflix' => $gdflix_slug,'name' => $name, 'link_info' => $link_info];
        
        $file = new File($pdo);
        $file->setLinkId($link_info['link_id']);
        
        if($file->checkServer(18)) {
            echo "Already Present in <a href='https://deaddrive.shop/file/".$link_info['uid'] . "'>" . $name . "</a><hr>";
            continue;
        }
        
        $file->storeServer(18,$gdflix_slug,"b081ff78b9fe1b375abc103bfd5dc3fa");
        
        echo "<a href='https://deaddrive.shop/file/".$link_info['uid'] . "'>" . $name . "</a><hr>";

        // print_r(json_encode($uid_gdflix));
    }
...
<textarea name="gdflix_links" cols="100" rows = "20"/>
</textarea>
