...
$files = $pdo->query("SELECT size,Name,views,downloads,uid,new_date FROM links_info WHERE user = $user_id $filter ORDER BY $sortby $order LIMIT $pgsize OFFSET $offset")->fetchAll(PDO::FETCH_ASSOC);
        ...
        <div class="card-body">
            <p class="mb-0">
                <center> <button id="panel" data-mdb-toggle="modal" data-mdb-target="#Panel" class="btn btn-info"
                        style="padding: .2rem 1rem;font-size: .7rem; line-height: 1.8;">Files Per Page</button>
                </center>
                <hr>
            </p>
                            ...
                            <tr>
                                <th scope="row">
                                    <input class="form-check-input" type="checkbox" id="select"
                                        value="<?PHP echo $file['uid'] ?>" fname="<?PHP echo $file['Name'] ?>"
                                        fsize="<?PHP echo $file['size'] ?>" />
                                </th>
                                <td>
                                    <div style="cursor: pointer;" onclick="copy('<?PHP echo $file['uid'] ?>')"><i
                                            class="fa fa-copy"></i></div>
                                </td>
                                <td>
                                    <a href="<?= $_ENV['PUBLIC_DOMAIN'] . '/file/' . $file['uid'] ?>" class="name">
                                ...
                                <td>
                                    <button class="btn btn-primary"
                                        onclick="rename('<?PHP echo $file['uid'] ?>', '<?PHP echo $file['Name'] ?>');"
                                        data-mdb-toggle="modal" data-mdb-target="#Rename"><i
