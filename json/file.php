...
$uid = $_GET['id'];
$sql = "SELECT users.site, users.site_name , Links_info.* , Servers.* FROM Links_info JOIN Servers ON Links_info.Id = Servers.Id Join users on Links_info.user = users.user_id WHERE Servers.uid ='$uid'";
...
foreach ($row as $key => $value) {
    ...
    if ($found_live && $value !== '' && $key !== 'uid' && $key != 'user') {
    $servers[$key] = array('id' => $value);
    ...
    $servers[$key][$subkey] = $subvalue;
        
    }
}
     }
$array = array('status' => 'true' , 'user' => $row['user'],  'site_name' => $row['site_name'], 'site' => $row['site'] ,'id' => $id, 'name' => $row['Name'], 'size' => $row['size'], 'extension' => $row['Type'], 'mimeType' => $row['mimeType'], 'duration' => $row['duration'], 'date' => $row['new_date'], 'live' => $row['live'], 'servers' => $servers);
