...
class User {
    ...
    public function UserApis($mode = "enabled") {
        ...
        if($mode == "all") {
            ...
            return $this->db->query("SELECT ua.* FROM user_apis ua JOIN server_info si ON si.server_id = ua.server_id WHERE enable = 1 AND disTem = 0 AND si.supportZip AND ua.user_id = " .$this->user_id)->fetchAll();
        }
    ...
    public function refreshServers() {
        ...
        $insertIntoApis = $this->db->prepare("INSERT INTO user_apis (user_id,server_id,server_name,server_domain,server_order) VALUES (:user_id,:server_id,:server_name,:server_domain,:server_order)");
        
        foreach($servers as $s) {
            
            $stmt = $this->db->prepare("SELECT 1 FROM user_apis WHERE user_id = :user_id AND server_id = :server_id");
            $stmt->execute(['user_id' => $this->getUserId(), 'server_id' => $s['server_id']]);
        ...
        return "Servers For User " . $this->getUserId() . " is Updated";
    }
    ...
    public function getUserServerDetails($server_id) {
       ...
       return [
        ...
        'server_domain' => $res['server_domain'],
        'apiLink' => $res['getApi'],
    ...
    public function setServerDomain($server_id,$server_domain) {
        
        $stmt = $this->db->prepare("UPDATE user_apis SET server_domain = :server_domain WHERE user_id = :user_id AND server_id = :server_id");
        
        $stmt->execute(['server_domain' => $server_domain, ':user_id' => $this->getUserId(), ':server_id' => $server_id]);
    }
    ...
    public function setServerApi($server_id,$api_fields) {
        ...
        if($server_id == 1) {
            ...
            $check = Neodrive::checkApi($api_fields['api'],$api_fields['email']);
            
        } else if($server_id == 19) {
            $upn = new UpnShare(UPNSHARE_API_URL,$api_fields['api']);
            $check = $upn->checkApi();
        } else if($server_id == 20) {
            $rpm = new RpmShare(RPMSHARE_API_URL,$api_fields['api']);
            $check = $rpm->checkApi();
        }
...
public function moveServer($server_id, $direction) {
    ...
    $stmt = $this->db->prepare($sql);
    $stmt->execute(['user_id' => $user_id, 'server_id' => $server_id]);
    ...
    $stmt = $this->db->prepare($sql);
    $stmt->execute(['user_id' => $user_id, 'current_order' => $currentOrder]);
    ...
    try {
        ...
        // Temporarily set to -1 to avoid unique conflict
        ...
        $this->db->rollBack();
        return ['status' => "error", 'message' => "Failed to swap servers: " . $e->getMessage()];
    }
}
}
