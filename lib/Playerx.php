...
class Playerx {
    ...
    public static function upload($api,$url) {
        ...
        $request = Fetch::getRequest(PLAYERX_API_URL . "?api_key=".$api."&url=".str_replace("'",'',$url)."&action=add_remote_url");
        ...
        if($request['status'] == "success") {
            ...
            if(!$request['result'] == true) {
                return ['status' =>"error", "message" => (isset($request['reason']) ? $request['reason'] : $request)];
            }
