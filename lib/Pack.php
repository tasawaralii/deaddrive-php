...
class Pack {
    private $db = null;
    private $Packquery = "SELECT li.link_id,li.Id,li.uid,li.Name FROM links_info li 
                    LEFT JOIN servers_links sl ON li.link_id = sl.link_id AND sl.server_id = :server_id 
                    WHERE li.live <> 0 AND sl.slug IS NULL AND li.user = 2 AND li.Type = 'zip'
                    ORDER BY li.link_id DESC LIMIT ";
                    
    private $allquery = "SELECT li.link_id,li.Id,li.uid,li.Name FROM links_info li 
                    LEFT JOIN servers_links sl ON li.link_id = sl.link_id AND sl.server_id = :server_id 
                    WHERE li.live <> 0 AND sl.slug IS NULL AND li.user = 2
                    ORDER BY li.link_id DESC LIMIT ";
    ...
    public function notHubcloud() {
        $this->Packquery .= "1";
        $res = $this->db->prepare($this->Packquery);
        $res->execute([':server_id' => 16]);
        $res = $res->fetchAll();
        
        if(!$res) {
            $this->allquery .= "2";
            $res = $this->db->prepare($this->allquery);
            $res->execute([':server_id' => 16]);
            $res = $res->fetchAll();
        }
